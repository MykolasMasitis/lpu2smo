PROCEDURE SelGuests
 ...
 IF MESSAGEBOX("ÎÒÎÁÐÀÒÜ ÓÑËÓÃÈ, ÎÊÀÇÀÍÍÛÅ"+CHR(13)+CHR(10)+"ÏÐÈÊÐÅÏËÅÍÍÛÌ Ê 'ÈÍÛÌ' ËÏÓ?"+CHR(13)+CHR(10),;
 ...
 m.BookName = m.repdir+'\sg'+m.gcperiod
 m.IsOpDoc = IsOpenExcelDoc('sg'+m.gcperiod)
 IF m.IsOpDoc
  IF !CloseExcelDoc('sg'+m.gcperiod)
   MESSAGEBOX('ÔÀÉË '+'sg'+m.gcperiod+' ÎÒÊÐÛÒ!',0+64,'')
   RETURN .f. 
  ENDIF 
 ENDIF 
 ...
 IF fso.FileExists(m.repdir+'\sg'+m.gcperiod+'.xls')
  fso.DeleteFile(m.repdir+'\sg'+m.gcperiod+'.xls')
 ENDIF  
 AllBook.SaveAs(m.repdir+'\sg'+m.gcperiod,18)
 oExcel.Visible=.t.
...
FUNCTION SelAlienOne()
 ...
 SCAN 
  ...
  m.docname = m.repdir+'\sg'+m.mcod+'.XLS'
  INSERT INTO curbooks (bkname) VALUES ('SG'+m.mcod+'.XLS')
