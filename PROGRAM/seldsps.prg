PROCEDURE SelDsps
 ...
 IF MESSAGEBOX("ÎÒÎÁÐÀÒÜ '×ÓÆÈÅ' ÄÈÑÏÀÍÑÅÐÈÇÀÖÈÈ?"+CHR(13)+CHR(10),;
 ...
 m.BookName = m.repdir+'\sf'+m.gcperiod
 m.IsOpDoc = IsOpenExcelDoc('sf'+m.gcperiod)
 IF m.IsOpDoc
  IF !CloseExcelDoc('sf'+m.gcperiod)
   MESSAGEBOX('ÔÀÉË '+'sf'+m.gcperiod+' ÎÒÊÐÛÒ!',0+64,'')
   RETURN .f. 
  ENDIF 
 ENDIF 
 ...
 IF fso.FileExists(m.repdir+'\sf'+m.gcperiod+'.xls')
  fso.DeleteFile(m.repdir+'\sf'+m.gcperiod+'.xls')
 ENDIF  
 AllBook.SaveAs(m.repdir+'\sf'+m.gcperiod,18)
 oExcel.Visible=.t.
...
FUNCTION SelAlienOne()
 ...
 SCAN 
  ...
  m.docname = m.repdir+'\sf'+m.mcod+'.XLS'
  INSERT INTO curbooks (bkname) VALUES ('sf'+m.mcod+'.XLS')
