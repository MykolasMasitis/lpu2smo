FUNCTION  AccReload2(lcmcod, lnlpuid, ismess)
 m.mcod  = lcmcod
 m.lpuid = lnlpuid
...
* fso.CopyFile(ZipDir+'\'+nsItem, m.bpath+'\'+nsItem)
 ...
 IF fso.FileExists(ZipDir+'\'+hoItem)
  fso.CopyFile(ZipDir+'\'+hoItem, m.bpath+'\'+hoItem)
 ENDIF 
 fso.DeleteFolder(ZipDir)
...
* REPLACE s_pred WITH m.s_pred, sum_flk WITH 0, paz WITH m.paz, nsch WITH m.nsch,;
 ...
 REPLACE s_pred WITH m.s_pred, sum_flk WITH 0, paz WITH m.paz, nsch WITH m.nsch,;
  ...
  krank WITH m.krank, paz_dst WITH m.paz_dst, paz_st WITH m.paz_st,;
  sum_kr WITH m.sum_kr, sum_dst WITH m.sum_dst, sum_st WITH m.sum_st,;
  usl_amb WITH m.usl_amb, kd_dst WITH m.kd_dst, ms_st WITH m.ms_st, erz_status WITH 0, erz_id WITH '' FOR mcod = m.mcod
...
FUNCTION CreateFilesStructure
 ...
 CREATE CURSOR curvmp (sn_pol c(25))
 INDEX ON sn_pol TAG sn_pol
 SET ORDER TO sn_pol
RETURN 
...
FUNCTION MakeHO
 tFile = m.bpath+'\'+hoItem
 IF !fso.FileExists(tFile)
  RETURN 
 ENDIF 
 oSettings.CodePage('&tFile', 866, .t.)
 =OpenFile(tFile, 'lcHOFile', 'excl')
 COPY TO &bpath\ho&qcod
 USE 
 USE &bpath\ho&qcod IN 0 ALIAS lcHOFile  EXCLUSIVE 
 SELECT lcHOFile
 INDEX on sn_pol+c_i+PADL(cod,6,'0') TAG unik
 USE 
 fso.DeleteFile(m.bpath+'\'+hoItem)
RETURN 
...
* oSettings.CodePage('&tFile', 866, .t.)
 ...
 USE (tFile)  IN 0 ALIAS lcSFile EXCLUSIVE 
...
*  SCATTER FIELDS lcDSFile.novor, lcDSFile.ds_s, lcDSFile.ds_p, lcDSFile.profil, lcDSFile.rslt,;
  ...
  DO CASE 
   ...
   CASE  IsKdS(m.cod) OR IsEko(m.cod)
   ...
   CASE IsVmp(m.cod)
*    m.sum_vmp = m.sum_vmp + m.s_all
    IF !SEEK(m.sn_pol, 'curvmp')
*     m.paz_vmp = m.paz_vmp + 1
     INSERT INTO curvmp (sn_pol) VALUES (m.sn_pol)
    ENDIF 
