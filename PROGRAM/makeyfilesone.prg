FUNCTION MakeYFilesOne
 ...
 PRIVATE m.lcpath
  ...
  SCAN 
   ...
   m.et        = 'A'
   m.et230     = 1 && Åñëè ÌÝÊ!
   m.oplata    = IIF(EMPTY(m.er_c), 1, 2) && 1-ïîëíàÿ îïëàòà, 2-ïîëíûé îòêàç, 3-÷àñòè÷íîå ñíÿòèå
   m.prmcod    = prmcod
...
*   m.s_all = fsumm(m.cod, m.tip, m.k_u, m.IsVed)
   ...
   DO CASE 
    ...
    DO CASE 
     CASE TipOfPr(m.mcod, m.prmcod) = 0 && íåïðèêðåïëåí
      m.f_type = 'fp'
*      IF m.IsLpuTpn = .T.
...
*       ENDIF 
*      ENDIF 
      CASE TipOfPr(m.mcod, m.prmcod) = 2 && ê ïèëîòó
*      IF (!EMPTY(m.lpu_ord) AND m.lpu_ord=m.prik) OR (EMPTY(m.lpu_ord) AND (m.lIs02=.T. OR INLIST(m.otd,'08','92')))
      IF !EMPTY(m.lpu_ord) OR (EMPTY(m.lpu_ord) AND (m.lIs02=.T. OR INLIST(SUBSTR(otd,2,2),'08','92')))
       m.f_type = 'vz' && âçàèìîçà÷åòû (èç ñðåäñòâ èíîãî ËÏÓ)
      ELSE 
       m.f_type = 'fp'
      ENDIF 
*      IF m.IsLpuTpn = .T.
...
*       ENDIF 
*      ENDIF 
      CASE TipOfPr(m.mcod, m.prmcod) = 3 && ñâîé
      m.f_type = 'fp'
*      IF m.IsLpuTpn = .T.
*       m.fil_id = fil_id
*       IF !SEEK(m.fil_id, 'lputpn', 'fil_id')
...
*  USE IN rerror
  ...
  IF m.sum_st1 != m.sum_st2
   MESSAGEBOX(''+CHR(13)+CHR(10)+;
    'ÂÍÈÌÀÍÈÅ!'+CHR(13)+CHR(10)+;
    'ÑÓÌÌÀ, ÐÀÑ×ÈÒÀÍÍÀß ÊÀÊ ÐÀÇÍÈÖÀ ÌÅÆÄÓ ÏÐÅÄÑÒÀÂËÅÍÍÎÉ È ÔËÊ, ÑÎÑÒÀÂËßÅÒ'+CHR(13)+CHR(10)+;
    TRANSFORM(m.sum_st1,'99 999 999.99')+CHR(13)+CHR(10)+;
    'ÑÓÌÌÀ ÏÎ ÄÀÍÍÛÌ ÏÅÐÑÎÍÈÔÈÖÈÐÎÂÀÍÍÎÉ ÎÒ×ÅÒÍÎÑÒÈ ÑÎÑÒÀÂËßÅÒ'+CHR(13)+CHR(10)+;
  ...
  IF m.nIsDoubles>0
   MESSAGEBOX('ÎÁÍÀÐÓÆÅÍÛ ÄÓÁËÈ Â ÐÅÃÈÑÒÐÅ!'+CHR(13)+CHR(10)+;
   m.stroka+'ÏÅÐÑÎÒ×ÅÒ ÍÅÂÅÐÅÍ!',0+64,m.mcod)
  ENDIF 
