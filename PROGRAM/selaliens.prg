PROCEDURE selaliens
 ...
 IF MESSAGEBOX("ОТОБРАТЬ УСЛУГИ, ОКАЗАННЫЕ"+CHR(13)+CHR(10)+"ПРИКРЕПЛЕННЫМ К 'ИНЫМ' ЛПУ?"+CHR(13)+CHR(10),4+32,'')=7
  RETURN 
 ENDIF 

 m.lpath = pbase+'\'+m.gcperiod
 ...
 CREATE CURSOR curaliens (period c(7), lpuid i(4), mcod c(7), sn_pol c(25), c_i c(25), ;
...
*    LOOP 
*   ENDIF 
   IF !EMPTY(people.prmcod) AND people.mcod!=people.prmcod
    m.sn_pol = sn_pol
    m.c_i    = c_i
    m.fam    = people.fam
    m.im     = people.im
...
*    INSERT INTO curaliens (period,lpuid,mcod,sn_pol,c_i,fam,im,ot,dr,w,;
     ul,dom,kor,str,kv,d_u,ds,otd,pcod, cod,k_u, s_all, d_beg, d_end, codname,lpuname) VALUES ;
